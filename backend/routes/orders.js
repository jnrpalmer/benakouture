...
const orderItems = items.map(function(item) {
  const p = products.find(function(x) { return x._id.toString() === item.productId; });
...
const subtotal = orderItems.reduce(function(sum, i) { return sum + i.price * i.quantity; }, 0);
...
for (let i = 0; i < items.length; i++) {
  await Product.findByIdAndUpdate(items[i].productId, { $inc: { stock: -items[i].quantity } });
}
